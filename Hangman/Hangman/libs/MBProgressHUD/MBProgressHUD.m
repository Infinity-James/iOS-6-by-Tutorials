...
- (void)layoutSubviews {
    ...
    if (showCancelButton) {
        ...
        // Move indicator to make room for the label
        indFrame.origin.y -= (floorf(bHeight / 2 + bPadding / 2));
        indicator.frame = indFrame;
        
        // Move first label to make room for the new label
        CGRect lFrame = label.frame;
        lFrame.origin.y -= (floorf(bHeight / 2 + bPadding / 2));
        label.frame = lFrame;
		
        // Set the label position and dimensions
        CGRect bFrame = CGRectMake(floorf((frame.size.width - bWidth) / 2) + xOffset,
                                   floorf(lFrame.origin.y + lFrame.size.height + bPadding),
                                   bWidth, bHeight);
        button.frame = bFrame;
        ...
        // Update height/width
        self.width = MAX(self.width, button.bounds.size.width + 2 * margin);                
        self.height = self.height + button.bounds.size.height + bPadding;
...
- (void)cancelTapped:(id)sender {
    ...
    [self hide:YES];
    if (delegate && [delegate respondsToSelector:@selector(hudWasCancelled:)]) {
        [delegate hudWasCancelled:self];
    }    
}
